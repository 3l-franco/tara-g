...
from services.sheets_client import get_ws, _api_call, read_df
...
def update_stock_and_log(product_id, product_name,
    ...
    headers = _api_call(ws.row_values, 1)
    ...
    current_val = _api_call(ws.cell, row_idx, stock_col).value
    try:
        current_val = int(float(str(current_val).strip() or '0'))
    ...
    _api_call(ws.update_cell, row_idx, stock_col, new_stock)
    log_transaction(product_name, action, old_stock,
                    new_stock, notes, username)
    return True


def log_transaction(product_name, action,
                    old_stock, new_stock, notes, username):
    ws = get_ws('transactions')
    now = datetime.now()
...
def update_product_in_sheet(product_id, product_name, updates: dict):
    ...
    for col_name, value in updates.items():
        if col_name in col_map:
            _api_call(ws.update_cell, row_idx, col_map[col_name], value)
...
with open('services/inventory_service.py', 'w', encoding='utf-8') as f:
    f.write(code)
print('inventory_service.py written OK')