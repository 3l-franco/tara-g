...
Add, edit thresholds/details, and delete products.
Mirrors admin manage_products but adapts to the staff mobile UI.
"""
...
def page_products_staff():
    """Staff product management — Add + List/Edit/Delete."""
...
def _tab_add():
    ...
    # ── Success overlay (shown after successful add) ──────
    added_name = st.session_state.pop('sp_add_success', None)
    if added_name:
        show_success_overlay('Product Added!', f'"{added_name}" saved.')
        return  # rerun triggered inside; safety return

    # Bump form_key on success so widgets reset to defaults
    ...
    with st.form(form_key):
        ...
        with c2:
            init_stock = st.number_input('Initial Stock',
                                         min_value=0, value=0,
                                         key='sp_add_init')
            min_stock  = st.number_input('Low Stock Threshold',
                                         min_value=1, value=10,
                                         key='sp_add_min')
            crit_stock = st.number_input('Critical Threshold',
                                         min_value=0, value=5,
                                         key='sp_add_crit')
    ...
    if submitted:
        if not name.strip():
            ...
            if (not ex.empty and
                    name.strip().lower() in
                    ex['product_name'].str.lower().values):
...
def _tab_list():
    ...
    if srch.strip():
        df = df[df['product_name'].str.contains(
            srch.strip(), case=False, na=False)]
        if df.empty:
    ...
    _TH = ('background:#F5EDD8;font-size:.65rem;font-weight:700;'
           'color:#6B5B3E;text-transform:uppercase;padding:5px 6px;')

    # Static header (HTML — no buttons here)
    st.markdown(
        f'<table style="width:100%;border-collapse:collapse;">'
        f'<thead><tr>'
        f'<th style="{_TH}">Product · Station · Stock</th>'
        f'<th style="{_TH}text-align:right;width:64px;"></th>'
        f'</tr></thead></table>',
        unsafe_allow_html=True)

    # ── One row per product: info (wide) + pencil btn ─────
    for idx, row in df.iterrows():
        ...
        c_info, c_btns = st.columns([7, 3], gap='small')
        c_info.markdown(
        ...
        with c_btns:
            ...
            if bc1.button('\u270e', key=f'sp_pencil_{idx}',
                ...
                st.rerun()
            if bc2.button('\U0001f5d1', key=f'sp_trash_{idx}',
        ...
        if st.session_state.get(ekey, False):
            _edit_panel(row, idx, pid, pname, STATIONS, ekey)

        if st.session_state.get(f'sp_cdel_{idx}', False):
            _delete_confirm(idx, pid, pname, ekey)


def _edit_panel(row, idx, pid, pname, STATIONS, ekey):
    with st.container(border=True):
        ...
        with bc:
            if st.button('Close', key=f'sp_close_{idx}',
